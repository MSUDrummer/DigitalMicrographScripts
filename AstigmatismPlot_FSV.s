...
class idcListenFFT
  ...
  void actOnImage(object self, image img)
    ...
    //if (num_x != num_y || ml2v != 0)
      ...
      //exit(0)
      //}
      // (num_x != num_y) not needed to allow flexibility of rectangular k2 images.
    ...
    if (doesImageExist(img_id)==0 && num_chg==0)
      {
      // deleteImage(prj_lns)
      prj_lns := createFloatImage( "Astigmatism Plot", hlf_mnr, num_wdg)
      }
    if (doesImageExist(img_id)==0 && num_chg>0)
      {
    ...
    for (cur_wdg=0; cur_wdg<num_wdg; cur_wdg++)
      ...
      // for AutoSurvey, set to 0,0 to allow change of scale/axes in DM
